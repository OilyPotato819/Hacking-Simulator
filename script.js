...
let stopwatch = {
   ...
   draw() {
      ...
      ctx.fillStyle = green;
      ctx.font = '50px Inconsolata';
      const y = this.y + this.h / 2 + 10.5;
      ctx.fillText(this.text[0], this.x + this.w / 3 / 2 - 25, y + 4.5);
      ctx.fillText(this.text[1], this.x + this.w / 2 - 25, y + 4.5);
      ctx.fillText(this.text[2], this.x + this.w * (5 / 6) - 25, y + 4.5);

      ctx.fillText(':', this.x - 12.5 + this.w * (2 / 3), y);
      ctx.fillText(':', this.x - 12.5 + this.w / 3, y);
   },

   updateText() {
      const centiseconds = Math.floor(this.time % 100);
      const seconds = Math.floor((this.time / 100) % 60);
      const minutes = Math.floor((this.time / 6000) % 99);

      ctx.font = '50px Inconsolata';
      this.text = [minutes.toString().padStart(2, '0'), seconds.toString().padStart(2, '0'), centiseconds.toString().padStart(2, '0')];
   },
...
function loop() {
   ...
   if (objects[1].outputs[0].num === objects[1].goals[0].num && (!objects[1].outputs[1] || objects[1].outputs[1].num === objects[1].goals[1].num)) {
      for (let i = 0; i < objects[0].goals.length; i++) {
