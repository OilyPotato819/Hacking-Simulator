...
objects[0].other.push(bigBox, regenerate, unknown, solves);
...
class Box {
   constructor(x, y, type) {
      ...
      if (type === 'input') {
         objects[0].inputs.push(this);
      } else if (type === 'output') {
         objects[0].outputs.push(this);
      } else {
         objects[0].goals.push(this);
      }
   }
...
class Line {
   constructor(lineStart, lineEnd, color, type) {
      ...
      if (type === 'input') objects[1].inputs.push(this);
      else if (type === 'output') objects[1].outputs.push(this);
      else if (type === 'goal') objects[1].goals.push(this);
   }
...
class Option {
   ...
   draw() {
      ...
      centerText(this.x + this.size / 2, this.y + this.size / 2, this.value, this.fontSize + 'px Inconsolata', this.color);
...
function resetBoxes() {
   ...
   for (let i = 0; i < objects[0].goals.length; i++) {
      const goalBox = objects[0].goals[i];
      goalBox.num = getResult(i, inputSolutions);
   }

   for (let i = 0; i < objects[0].inputs.length; i++) {
      objects[0].inputs[i].num = randInt(0, 9);
      objects[0].inputs[i].revealed = false;
   }

   for (let i = 0; i < objects[1].goals.length; i++) {
      objects[1].goals[i].color = 'red';
   }
...
function updateOutputs() {
   for (let i = 0; i < outputNum; i++) {
      const output = objects[0].outputs[i];
      const inputs = objects[0].inputs.map((obj) => obj.num);
...
function revealInput() {
   const unrevealed = objects[0].inputs.filter((obj) => !obj.revealed);
...
function loop() {
   ...
   if (objects[0].outputs[0].num === objects[0].goals[0].num && (!objects[0].outputs[1] || objects[0].outputs[1].num === objects[0].goals[1].num)) {
      for (let i = 0; i < objects[1].goals.length; i++) {
         objects[1].goals[i].color = green;
         solved = true;
